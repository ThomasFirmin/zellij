...
class Searchspace:
    ...
    def __init__(self, labels, types, values, neighborhood=0.10):
        ...
        assert len(labels) == len(types) == len(values), "Labels, types and values must have the same length"
        ...
        for v, t in zip(values, types):
            if t == "R":
                ...
                assert (
                    isinstance(v, list) and len(v) == 2 and isinstance(v[0], int) and isinstance(v[1], int) and v[0] < v[1]
                ), f"Values of type 'D' must be of the form [a : int, b : int] and b > a, got {v}"

            elif t == "C":
                assert isinstance(v, list) and len(v) >= 2, f"Values of type 'C' must be of the form [value 1, value 2,...], got {v}"

        if isinstance(neighborhood, list):
            for n, t in zip(neighborhood, types):
                if t == "R":
                    assert isinstance(n, float) or isinstance(n, int), f"neighborhood of type 'R' must be a float or an int, got {n}"

                elif t == "D":
                    assert isinstance(n, int), f"neighborhood of type 'R' must be an int, got {n}"

                else:
                    assert n == -1, f"neighborhood of type 'C' or 'K' must be equal to -1, got {n}"
    ...
    def subspace(self, lo_bounds, up_bounds):
        ...
        subspace = Searchspace(self.labels, new_types, new_values, new_neighborhood)
    ...
    def show(self, X, Y, save=False, path=""):
        ...
        for i in range(self.n_variables):
            ...
            if i > 0:
                ...
                if self.types[i] == "C":
                    ...
                    plots[i, i].tick_params(axis="y", labelsize=7 / len(self.types[i]))
            ...
            for j in range(i + 1, self.n_variables):
                ...
                if self.types[i] == "C" or self.types[j] == "C":
                    ...
                    if self.types[i] == self.types[j]:
                        ...
                        plots[j, i].boxplot(
                            data, vert=vert, flierprops=dict(marker="o", markerfacecolor="green", markersize=0.1, markeredgecolor="green"), labels=self.values[idx]
                        )
        ...
        plt.subplots_adjust(left=0.050, bottom=0.050, right=0.970, top=0.970, wspace=0, hspace=0)
