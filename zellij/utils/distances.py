...
class Euclidean(Distance):
    ...
    Example
    -------
    >>> from zellij.core import Loss, ArrayVar, FloatVar, ContinuousSearchspace
    >>> from zellij.utils import Euclidean
    >>> from zellij.utils.benchmark import himmelblau
    >>> lf = Loss()(himmelblau)
    >>> values = ArrayVar(FloatVar("float_1", 0,1),
    ...              FloatVar("float_2", 0,1))
    >>> sp = ContinuousSearchspace(values,lf, distance=Euclidean())
...
class Mixed(Distance):
    ...
    def target(self, object):
        ...
        ), logger.error(f"Target must be of type `MixedSearchspace`, got {object}")
        if self._target:
            if self.weights:
