...
# @Last modified time: 2023-03-31T16:22:21+02:00
...
class NeighborMutation(Mutator):
    ...
    def target(self, search_space):
        if search_space:
            ...
            assert all(
                hasattr(val, "neighbor") for val in search_space.variables
            ), f"""For {self.__class__.__name__} variables of target object must
            have a `neighbor` method. When defining the :ref:`sp`,
            user must define the `neighbor` kwarg before the `mutation` kwarg.
            ex:\n
            >>> ContinuousSearchspace(variables, loss, neighbor=..., mutation=...)
            """
    ...
    def __call__(self, individual):
        ...
        for val in self.target.variables:
            if np.random.random() < self.probability and not isinstance(val, Constant):
