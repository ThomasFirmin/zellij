...
class CASCBO(UnitMetaheuristic):
    ...
    def forward(
        ...
        if not self.initialized:
            ...
            raise InputError(
                "After initialization SCBO must receive non-empty X, Y, secondary and constraint in forward."
            )
        else:
            ...
            if len(self.train_obj) < self.initial_size:
                ...
                with gpytorch.settings.max_cholesky_size(self.cholesky_size):
                    ...
                    # Update Cost model
                    cost_model = CostModelGP(self.train_x, self.train_cost)
