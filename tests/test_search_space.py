...
class TestSearchSpace(unittest.TestCase):
    ...
    def test_random_attribute(self):
        self.assertIn(self.sp.random_attribute(),self.sp.label,\
        "Wrong Random attribute with size=1, replace=True, exclude=None")

        self.assertIn(self.sp.random_attribute(exclude="a"),self.sp.label[1:],\
    ...
    def test_random_value(self):
        self.assertTrue(isinstance(self.sp.random_value("a")[0],float) and (-5<=self.sp.random_value("a")[0]<=5),\
        "Wrong Random real value with size=1, replace=True, exclude=None")

        self.assertTrue(isinstance(self.sp.random_value("b")[0],int) and (-5<=self.sp.random_value("b")[0]<=5),\
        "Wrong Random int value with size=1, replace=True, exclude=None")

        self.assertIn(self.sp.random_value("c")[0],self.sp.values[2],\
        "Wrong Random categorical value with size=1, replace=True, exclude=None")

        self.assertIn(self.sp.random_value("c",exclude="v1")[0],self.sp.values[2][1:],
        "Wrong Random attribute with exclusion")
    ...
    def test_get_categorical_neighbor(self):
        ...
        self.assertTrue(\
        neighbor in ["v1","v3"],\
        "Wrong categorical neighbor generation")
    ...
    def test_get_neighbor(self):
        ...
        self.assertTrue(\
        (isinstance(neighbor[0],float)\
        or isinstance(neighbor[0],int))\
    ...
    def test_convert(self):
        ...
        self.assertEqual(converted,[0.9, 0.9, 0.3333333333333333, 1],\
        "Wrong convertion to continuous")
        self.assertEqual(reconverted,[4,4,"v2",2],\
        "Wrong convertion to mixed")

        converted = self.sp.convert_to_continuous([[5,5,"v3",2]])[0]
        reconverted = self.sp.convert_to_continuous([converted],reverse=True)[0]

        self.assertEqual(converted,[1, 1, 0.6666666666666666, 1],\
        "Wrong convertion to continuous of the upper bounds")
        self.assertEqual(reconverted,[5,5,"v3",2],\
        "Wrong convertion to mixed of the upper bounds")

        converted = self.sp.convert_to_continuous([[-5,-5,"v1",2]])[0]
        reconverted = self.sp.convert_to_continuous([converted],reverse=True)[0]

        self.assertEqual(converted,[0, 0, 0,1],\
        "Wrong convertion to continuous of the lower bounds")
        self.assertEqual(reconverted,[-5,-5,"v1",2],\
        "Wrong convertion to mixed of the lower bounds")

    def test_general_convert(self):
        spc = self.sp.general_convert()

        self.assertEqual(spc.label,self.sp.label,\
        "Wrong general convertion for labels")
        self.assertTrue(all(x == "R" for x in spc.type),\
        "Wrong general convertion for types")
        self.assertTrue(all(x == [0,1] for x in spc.values),\
        "Wrong general convertion for values")

        self.assertEqual(spc.neighborhood,[0.05, 0.1, 1, 1],\
    ...
    def test_subspace(self):
        ...
        self.assertEqual(spc.values,[[-2.0, 2.0], 2, ['v2', 'v3'], 2],\
        "Wrong subspacing for values")
        self.assertEqual(spc.type,['R', 'K', 'C', 'K'],\
        "Wrong subspacing for types")
        self.assertEqual(spc.neighborhood,[0.2, -1, -1, -1],\
        ...
        self.assertEqual(spc.values,[[-5.0, -4.99], [-5, -4], ['v1', 'v2', 'v3'], 2],\
        "Wrong subspacing for lower bounds - values")
        self.assertEqual(spc.type,['R', 'D', 'C', 'K'],\
        "Wrong subspacing for lower bounds - types")
        self.assertEqual(spc.neighborhood,[0.0004999999999999894, 1, -1, -1],\
        ...
        self.assertEqual(spc.values,[[4.99, 5.0], [4, 5], 'v1', 2],\
        "Wrong subspacing for upper bounds")
        self.assertEqual(spc.type,['R', 'D', 'K', 'K'],\
